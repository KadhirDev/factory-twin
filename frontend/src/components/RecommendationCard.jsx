...
function scoreAction(rule, ctx, confidenceLabel) {
  ...
  const corrPts =
    rule.correlationKey && ctx.correlatedPairs.includes(rule.correlationKey) ? 15 : 0;

  const metricKey = rule.id.split("_")[0];
  const eta = ctx.etaMap[metricKey]?.etaCrit;
  const etaPts =
    eta == null ? 0 : eta <= 5 ? 30 : eta <= 15 ? 20 : eta <= 30 ? 10 : 0;
...
export default function RecommendationCard({
  ...
  const recommendations = useMemo(() => {
    ...
    const matched = RULES.filter((rule) => {
      ...
      .map((rule) => ({
        ...rule,
        actionScore: scoreAction(rule, ctx, confidenceLabel),
      }))
  ...
  }, [telemetry, correlations, etaItems, anomalyScore, confidenceLabel]);
