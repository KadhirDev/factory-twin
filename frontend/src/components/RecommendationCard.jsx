 ...
 *
 * New: accepts optional `causeSummary` prop — when provided, the
 * "Recommended First Action" banner shows "Responding to: X" to visually
 * connect the detected cause to the recommended action.
 */
...
const URGENCY_STYLE = {
  critical: {
    border: "border-red-300",    bg: "bg-red-50",    stripe: "bg-red-500",
    title:  "text-red-800",      text: "text-red-700",
    badge:  "bg-red-100 text-red-700 border-red-300",   icon: "text-red-500",
  },
  warning: {
    border: "border-orange-300", bg: "bg-orange-50", stripe: "bg-orange-400",
    title:  "text-orange-800",   text: "text-orange-700",
    badge:  "bg-orange-100 text-orange-700 border-orange-300", icon: "text-orange-500",
  },
  info: {
    border: "border-green-200",  bg: "bg-green-50",  stripe: "bg-green-400",
    title:  "text-green-800",    text: "text-green-600",
    badge:  "bg-green-100 text-green-700 border-green-200",  icon: "text-green-500",
  },
};
...
function buildContext(telemetry, correlations, etaItems, anomalyScore) {
  ...
  const etaMap = {};
  (etaItems || []).forEach((e) => { etaMap[e.key] = e; });
  return {
    metrics: {
 ...
 *   causeSummary  – string | null — short description of detected cause for
 *                   cross-component alignment display in banner
      ...
      {firstAction && firstAction.id !== "all_clear" && (
        <div className="mb-2 rounded-xl border-2 border-indigo-300 bg-indigo-50 overflow-hidden">
          ...
          <div className="px-3 py-3">
            {/* ── Cause→Action link — shown when causeSummary is available ── */}
            {causeSummary && (
            ...
            {firstAction.why && (
              <p className="text-[10px] text-indigo-500 border-t border-indigo-200 pt-1.5 mb-1">
                <span className="font-semibold">Why first: </span>{firstAction.why}
              </p>
            )}
                ...
                <div className="flex-1 px-3 py-3">
                  <div className="flex items-start justify-between gap-2 mb-1">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <span className="text-[10px] font-mono text-gray-400">{rec.actionScore}</span>
                      {rec.impact && (
                  ...
                  {rec.why && (
                    <p className="text-[10px] text-gray-400 mt-1 flex items-start gap-1">
                      ...
                      <span><span className="font-medium">Why: </span>{rec.why}</span>
                    </p>
                  )}
                  {rec.consequence && (
                    <>
