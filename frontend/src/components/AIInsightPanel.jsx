...
function etaMinutes(current, slope, threshold, inverted = false) {
  const headingToward = inverted
    ? slope < 0 && current > threshold
    : slope > 0 && current < threshold;
  if (!headingToward || Math.abs(slope) < 1e-6) return null;
...
function detectCorrelations(telemetry = []) {
  ...
  return CORRELATION_PAIRS.filter(({ a, b }) => {
    ...
    return (
      aVals.length >= 8 &&
      bVals.length >= 8 &&
      detectTrend(aVals) === "rising" &&
      detectTrend(bVals) === "rising"
    );
  });
}
...
function computeAnomalyFrequency(telemetry = []) {
  ...
  const newerRate = newerCount / newer.length;
...
function computeRiskIndex(anomalyScore, freqData, etaItems) {
  ...
  if (critETAs.length > 0) {
    const minCritETA = Math.min(...critETAs);
    if (minCritETA <= 5) etaComponent = 30;
    else if (minCritETA <= 15) etaComponent = 20;
    else if (minCritETA <= 30) etaComponent = 10;
  }
...
function SeverityBadge({ score }) {
  ...
  return (
    <span
      className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${sev.bg} ${sev.color} ${sev.border}`}
    >
      {sev.label}
    </span>
  );
  ...
  return (
    <div className="bg-gray-50 border border-gray-100 rounded-lg p-3 mb-4">
      ...
      <p className="text-xs text-gray-400 mt-1 text-right">
        {topMetric.pct}% of critical threshold
      </p>
    </div>
  );
}
              ...
              <div className="flex items-center justify-between text-xs mb-0.5">
                ...
                <span
                  className={`font-mono text-xs ${
                    absZ >= 4.0 ? "text-red-700" : "text-orange-600"
                  }`}
                >
                  {c.z_score > 0 ? "+" : ""}
    ...
    <div className="mb-4">
      ...
      <div className="space-y-2">
        {etas.map((eta) => {
          ...
          const Icon = urgencyCfg.icon;
          const barPct = Math.min(100, Math.max(0, eta.fraction * 100));

          return (
            <div
              key={eta.key}
              className={`rounded-lg border p-3 ${urgencyCfg.bg} ${urgencyCfg.border}`}
            >
              <div className="flex items-center justify-between mb-1.5">
                <div className="flex items-center gap-2">
                  <Icon size={13} className={urgencyCfg.text} />
                  <span className={`text-xs font-semibold ${urgencyCfg.text}`}>
                    {eta.label}
                  </span>
                </div>
                <span className={`text-xs font-mono font-bold tabular-nums ${urgencyCfg.text}`}>
              ...
              <div className="h-1.5 bg-white rounded-full overflow-hidden border border-gray-100 mb-2">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${urgencyCfg.fill}`}
                  style={{ width: `${barPct}%` }}
                />
              </div>
              ...
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-xs font-semibold bg-purple-100 text-purple-800 border border-purple-300 rounded-full px-2 py-0.5 flex items-center gap-1">
                    <TrendingUp size={9} />
                    {METRIC_CONFIG[pair.a]?.label ?? pair.a}
                  </span>
                  <span className="text-xs text-purple-400">+</span>
                  <span className="text-xs font-semibold bg-purple-100 text-purple-800 border border-purple-300 rounded-full px-2 py-0.5 flex items-center gap-1">
                    <TrendingUp size={9} />
                    {METRIC_CONFIG[pair.b]?.label ?? pair.b}
                  </span>
    ...
    <div className={`rounded-lg border p-3 mb-4 ${bgClass}`}>
      <div className="flex items-center justify-between mb-2">
        ...
        <div className={`flex items-center gap-1 text-xs font-bold ${trendColor}`}>
          <TrendIcon size={12} />
          <span className="capitalize">{freq.trend}</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4 text-xs">
        ...
        <div className="flex gap-4 text-xs text-gray-500">
          <div>
            <span className="font-medium text-gray-700">{freq.recentCount}</span>
            <span className="ml-1">recent half</span>
          </div>
          <div>
            <span className="font-medium text-gray-700">{freq.olderCount}</span>
            <span className="ml-1">older half</span>
          </div>
        </div>
      </div>
...
function RiskIndexBar({ riskData }) {
  if (!riskData) return null;
  const { index, breakdown, level } = riskData;
  const rc = RISK_COLORS[level] || RISK_COLORS.normal;

  return (
    <div className={`rounded-xl border p-4 mb-4 ${rc.bg} border-gray-200`}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
      ...
      <div className="h-3 bg-white rounded-full overflow-hidden border border-gray-100 mb-2 flex">
        <div
          ...
          style={{ width: `${breakdown.anomalyScore}%` }}
          title={`Anomaly score: ${breakdown.anomalyScore}pts`}
        />
        <div
          className="h-full bg-orange-400 transition-all duration-500"
          style={{ width: `${breakdown.frequency}%` }}
          title={`Frequency trend: ${breakdown.frequency}pts`}
        />
        <div
          className="h-full bg-yellow-400 transition-all duration-500"
          style={{ width: `${breakdown.etaUrgency}%` }}
          title={`ETA urgency: ${breakdown.etaUrgency}pts`}
        />
      </div>
      ...
      <div className="flex gap-3 text-[10px] text-gray-500">
        <span className="flex items-center gap-1">
          ...
          Score ({breakdown.anomalyScore}pts)
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-orange-400 inline-block" />
          Frequency ({breakdown.frequency}pts)
        </span>
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-yellow-400 inline-block" />
          ETA ({breakdown.etaUrgency}pts)
        </span>
      </div>
    </div>
  );
...
// ─────────────────────────────────────────────────────────────────────────────
// NEW: PRIORITY INSIGHTS
// ─────────────────────────────────────────────────────────────────────────────

const DEFAULT_VISIBLE = 3;

function PriorityInsights({ insights }) {
  ...
  const sorted = [...insights].sort((a, b) => {
    const pa = (a.priority ?? 50) + (TYPE_WEIGHT[a.type] ?? 2) * 10;
    const pb = (b.priority ?? 50) + (TYPE_WEIGHT[b.type] ?? 2) * 10;
    return pa - pb;
  });
...
export default function AIInsightPanel({
  ...
  const insights = useMemo(
    ...
    [telemetry, anomalies, anomalyStats]
  );
  ...
  const riskData = useMemo(
    ...
    [latestScore, freqData, currentETAs]
  );
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow p-5">
      ...
      <RiskIndexBar riskData={riskData} />
      ...
      <div className="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between text-xs text-gray-400">
        <span>
          Anomalies stored: <strong className="text-gray-600">{totalAnomalies}</strong>
        </span>
        <span>
