...
// ─────────────────────────────────────────────────────────────────────────────
// SIGNAL ICONS — maps signal type → lucide icon for the explainability panel
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// CONFIDENCE (7-signal calibrated engine)
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// INSIGHT GENERATION — each insight carries a `signals` array explaining why
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// COHERENCE FILTER
// Removes contradictory insights: "all clear" cannot coexist with actionable
// warnings or critical findings.
// ─────────────────────────────────────────────────────────────────────────────
...
function filterCoherent(insights) {
  ...
  if (hasActionable) {
    return insights.filter((i) => i.type !== "ok");
  }
  return insights;
}
...
// ── SignalRow — one contributing signal inside the "Why?" section ────────────
...
// ── InsightRow — shows text + confidence tag + expandable "Why?" section ─────
  ...
  return (
    <div className={`flex items-start gap-3 rounded-lg border p-3 ${s.wrap}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Insight text + expandable "Why?" toggle */}
        ...
        {/* Contributing signals (expandable "Why?" section) */}
...
// ─────────────────────────────────────────────────────────────────────────────
// PRIORITY INSIGHTS — coherence + stability + sorted + noise-suppressed
// ─────────────────────────────────────────────────────────────────────────────
...
function PriorityInsights({ insights, riskIndex = 0 }) {
  ...
  // Step 1: Remove contradictory "all clear" when actionable insights exist
  const coherentInsights = useMemo(() => filterCoherent(insights), [insights]);

  // Step 2: Stabilize ordering — only commit a reorder when meaningful changes occur
  // (new critical appears, critical disappears, large count change, or 8s elapsed)
  const stableInsights = useStableInsights(coherentInsights);

  if (!stableInsights?.length) return null;

  // Step 3: Sort by severity → urgency priority → confidence
  ...
  // Step 4: Suppress low-confidence noise in stable, low-risk state
...
export default function AIInsightPanel({
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow p-5">
      ...
      {/* ── Priority insights — coherent, stable, sorted ───────────────────── */}
